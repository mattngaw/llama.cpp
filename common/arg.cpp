    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            params.out_file = value;
                    ...
                    LLAMA_EXAMPLE_RESULTS, LLAMA_EXAMPLE_EXPORT_GRAPH_OPS, LLAMA_EXAMPLE_GRAPH_VIZ}));
    add_opt(common_arg(
        {"--full"},
        "emit the full unrolled graph instead of collapsing repeated layers",
        [](common_params & params) {
            params.graph_full = true;
        }
    ).set_examples({LLAMA_EXAMPLE_GRAPH_VIZ}));
    add_opt(common_arg(
